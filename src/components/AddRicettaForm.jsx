...
const AddRicettaForm = () => {
  ...
  const addRicetta = async function(){
    ...
    const response = await fetch("https://aipluswebserver-vincenzocostantini-082c8784.koyeb.app/customer/createcustomer/ricette/add", {
      method: 'POST', 
      headers: {
  ...
  const handleGinFlavourModalSubmit = async () => {
    try {
      const response = await fetch('https://aipluswebserver-vincenzocostantini-082c8784.koyeb.app/customer/createcustomer/ginflavours/add', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${sessionStorage.getItem('token')}`,
          'Content-Type': 'application/json'
