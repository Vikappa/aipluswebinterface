...
const GinBrandsWindow = () => {
    ...
    const fetchGinBrands = async () => {
        try {
            const response = await fetch('http://localhost:3001/admin/ginbrand/getall', {
                method: 'GET',
                headers: {
    ...
    const addNewBrand = async ({ name, description, imageUrl, sovrapprezzo }) => {
        ...
        try {
            const response = await fetch('http://localhost:3001/admin/ginbrand', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': 'Bearer ' + sessionToken,
    ...
    const addNewBrandWithImg = async ({ name, description, image, surcharge }) => {
        ...
        try {
            const response = await fetch('http://localhost:3001/admin/ginbrand/uploadimage', {
                method: 'POST',
                headers: {
                    'Authorization': 'Bearer ' + sessionToken,
                },
